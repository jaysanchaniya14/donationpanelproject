...
@section('title', config('app.name') .' | Project - News feeds')
...
@section('top-bar')
    <h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Project News Feed</h1>
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-dark">Projects News feed</li>
    </ul>
@endsection
    ...
    <script>
        ...
        var NewsFeedTable = function () {
            ...
            var initDatatable = function () {
                dt = $("#newsfeed-table").DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ route('admin.projects.newsfeed', ['id' => $project->id]) }}",
                    },
                    columns: [
        ...
        $("#edit-newsfeed-image").change(function () {
            ...
            files.forEach(file => {
                var ext = file.name.split('.').pop().toLowerCase();
                if (valid_img_ext.includes(ext) &&  $(".project-img-preview").length < 5 && !video) {
                    if($("#edit-media-preview").find('video').length > 0){
                        valid_files = [];
                        total_size = 0;
                        $("#edit-media-preview").html('');
                    }
